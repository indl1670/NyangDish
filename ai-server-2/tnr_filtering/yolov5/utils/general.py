...
if platform.system() == 'Windows':
    for fn in LOGGER.info, LOGGER.info:
...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if new_size != imgsz:
        LOGGER.info(f'WARNING ⚠️ --img-size {imgsz} must be multiple of max stride {s}, updating to {new_size}')
    return new_size
...
def check_imshow(warn=False):
    ...
    try:
        ...
        if warn:
            LOGGER.info(f'WARNING ⚠️ Environment does not support cv2.imshow() or PIL Image.show()\n{e}')
        return False
...
def check_amp(model):
    ...
    try:
        ...
        LOGGER.info(f'{prefix}checks failed ❌, disabling Automatic Mixed Precision. See {help_url}')
        return False
        ...
        if os.path.isfile(url):
            ...
            for i in range(retry + 1):
                ...
                if success:
                    ...
                    LOGGER.info(f'⚠️ Download failure, retrying {i + 1}/{retry} {url}...')
                else:
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if (time.time() - t) > time_limit:
            LOGGER.info(f'WARNING ⚠️ NMS time limit {time_limit:.3f}s exceeded')
